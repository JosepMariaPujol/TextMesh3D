...
namespace Softgraph.TextMesh3D.Runtime
{
    public static class HalfEdgeHelpMethods
    {
        ...
        public static void FlipTriangleEdge(HalfEdge2 e)
        {
            ...
            Vector2 aPos = e_1.v.position;
            Vector2 bPos = e_2.v.position;
            Vector2 cPos = e_3.v.position;
            Vector2 dPos = e_5.v.position;
        ...
        public static void SplitTriangleFaceAtPoint(HalfEdgeFace2 f, Vector2 splitPosition, HalfEdgeData2 data)
        {
            ...
            foreach (HalfEdge2 e in newEdges)
            {
                ...
                Vector2 eGoingTo = e.v.position;
                Vector2 eGoingFrom = e.prevEdge.v.position;
                ...
                foreach (HalfEdge2 eOpposite in newEdges)
                {
                    ...
                    Vector2 eGoingToOther = eOpposite.v.position;
                    Vector2 eGoingFromOther = eOpposite.prevEdge.v.position;

                    if (eGoingTo.Equals(eGoingFromOther) && eGoingFrom.Equals(eGoingToOther))
                    {
        ...
        private static void CreateNewFace(HalfEdge2 e_old, Vector2 splitPosition, HalfEdgeData2 data, HashSet<HalfEdge2> newEdges)
        {
            //This triangle has the following positons
            Vector2 p_split = splitPosition;
            Vector2 p_next = e_old.prevEdge.v.position;
            Vector2 p_prev = e_old.v.position;
